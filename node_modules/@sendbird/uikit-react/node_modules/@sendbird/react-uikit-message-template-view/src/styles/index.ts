...
export function setViewProps(
  ...
  if (props.viewStyle?.padding) {
    const padding = props.viewStyle.padding;
    if (isNumber(padding.top)) padding.top = Number(padding.top);
    if (isNumber(padding.bottom)) padding.bottom = Number(padding.bottom);
    if (isNumber(padding.left)) padding.left = Number(padding.left);
    if (isNumber(padding.right)) padding.right = Number(padding.right);
  }
}
...
export function setViewStyle(styles: ReactParsedProperties, props: ComponentsUnion['properties']) {
  ...
  if (isNumber(viewStyle?.margin?.top)) styles['marginTop'] = Number(viewStyle?.margin?.top);
  if (isNumber(viewStyle?.margin?.bottom)) styles['marginBottom'] = Number(viewStyle?.margin?.bottom);
  if (isNumber(viewStyle?.margin?.left)) styles['marginLeft'] = Number(viewStyle?.margin?.left);
  if (isNumber(viewStyle?.margin?.right)) styles['marginRight'] = Number(viewStyle?.margin?.right);

  const marginHorizontal = Number(styles.marginLeft ?? '0') + Number(styles.marginRight ?? '0');
...
export function setTextStyle(styles: ReactParsedProperties, props: Text | TextButton, options: ParserMapOptions) {
  ...
  if (width?.value == FlexSizeSpecValue.WrapContent) {
    styles['maxWidth'] = '100%';
  }
